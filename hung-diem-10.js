    ...
    // ── STATE ───────────────────────────────────────────────
    let state        = 'idle';
    let allFaces     = [];       // all faces detected this frame
    let selectedFace = null;     // chosen by mouse click in selecting mode
    let playerFace   = null;     // tracked during gameplay
    let score        = 0;
    let lives        = 3;
    let items        = [];
    let effects      = [];
    ...
    let noFaceTimer  = 0;
    let basket       = null;
    let gameStartTime= 0;        // 0 = not running
    let gamePausedAt = 0;        // frozen timestamp when game ended
    let currentLevel = 1;
    let levelBanner  = null;
    let flashColor   = null, flashAlpha = 0;

    // ── LEVEL CONFIG ────────────────────────────────────────
    // Each level lasts 15s. Levels go infinite.
    const LEVEL_BASE = [
      null,
    ...
    // Elapsed seconds — frozen after game ends
    function elapsedSec() {
    ...
    async function initCamera() {
      try {
        ...
        det.onResults(results => {
          ...
          // During play, re-lock playerFace to the closest tracked face
          if (state === 'playing' && playerFace) {
            let best=null, bestD=Infinity;
            for (const f of allFaces) {
              const d = Math.abs(f.cx - playerFace.cx);
              if (d < bestD) { bestD=d; best=f; }
            }
            if (best && bestD < W*0.35) playerFace = best;
          }
        });
    ...
    function renderSelecting() {
      ...
      allFaces.forEach((f, i) => {
        ...
        // Number badge (top-right corner of face box)
        ...
        // Basket preview on selected face
    ...
    function renderPlaying() {
      ...
      if (noFaceTimer > 30) {
        ctx.fillStyle='rgba(255,78,78,.12)'; ctx.fillRect(0,0,W,H);
        ctx.fillStyle='rgba(255,78,78,.88)';
        ctx.font=`bold ${Math.round(W*.022)}px Nunito,sans-serif`;
        ctx.textAlign='center'; ctx.textBaseline='top';
        ctx.fillText('⚠️ Quay lại camera!', W/2, 72);
      }
    }
    ...
    function drawBasket(cx, faceTopY, bw) {
      ...
      ctx.closePath(); ctx.fillStyle='rgba(190,130,55,.9)'; ctx.fill(); ctx.shadowBlur=0;
      // Weave H
      for(let i=1;i<5;i++){
    ...
    function updateItems() {
      ...
      for(let i=items.length-1;i>=0;i--){
        const it=items[i]; it.y+=it.speed; it.rot+=it.rotSpd;
        // Collision
        ...
        ctx.shadowColor=it.bad?'#ef4444':it.border; ctx.shadowBlur=16;
        ...
        ctx.shadowBlur=0; ctx.fillStyle=it.txt;
    ...
    function startGame() {
      ...
      playerFace=selectedFace; score=0; lives=3;
    ...
    function endGame() {
      gamePausedAt = Date.now();   // ← freeze the clock
